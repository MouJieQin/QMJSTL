...
struct avl_tree_base_node {
  ...
  avl_tree_base_node(link_type p = nullptr, link_type left = nullptr,
                     link_type right = nullptr, int height = 0)
...
template <typename value_type>
struct avl_tree_node : avl_tree_base_node<value_type> {
  typedef avl_tree_node<value_type>* link_type;
  typedef avl_tree_base_node<value_type> base_type;

  avl_tree_node(const value_type& value, link_type p = nullptr,
                link_type left = nullptr, link_type right = nullptr,
                int height = 0)
      : value(value), base_type(p, left, right, height) {}

  avl_tree_node(value_type&& value, link_type p = nullptr,
                link_type left = nullptr, link_type right = nullptr,
                int height = 0)
...
class avl_tree {
 public:
  ...
  link_type insert(const value_type& val) { return (insert_equal(val)); }
  ...
  void erase(link_type tar) { delete_node(tar); }

  void print() { print(root, 0); }

  void print_height() { cout << "height=" << root->height << endl; }
  ...
  void print_sorted() {
    ...
    for (; cur;) {
      ...
      if (cur->right)
        ...
        while (cur != root && cur->p->right == cur) cur = cur->p;
        cur = cur->p;
      }
    }
  }
 ...
 private:
  static link_type minimum(link_type node) {
    for (link_type left = node->left; left; left = node->left) node = left;
    return (node);
  }
  ...
  void insert_imple(link_type par, link_type tar, const value_type& val) {
    ...
    if (new_height <= par->height) return;
    par->height = new_height;
    if (par != root) insert_fixup(par->p, val);
  }
    ...
    for (link_type par = cur; cur; cur = par) {
      ...
      if (val < cur->value) {
        if (left_height > right_height) {
          ...
          if (new_height <= cur->height) return;
  ...
  void delete_fixup(link_type tar) {
    ...
    for (link_type par = tar; par;) {
      ...
      if (right_height < left_height) {
        if (left_height - right_height == 2) {
          if (height(left->right) > height(left->left)) left_rotate_del(left);
          right_rotate_del(tar);
          return;
        }
        new_height = left_height + 1;
        if (new_height <= tar->height) return;
 ...
 private:
  ...
  void destroy_tree(link_type cur) {
    if (cur->left) destroy_tree(cur->left);
    if (cur->right) destroy_tree(cur->right);
    destroy_and_free_node(cur);
  }

  int height(link_type tar) { return (tar ? tar->height : -1); }
  ...
  void left_rotate_imple(link_type tar, link_type tarRight) {
    ...
    if (tarRight->left) tarRight->left->p = tar;
  ...
  void right_rotate_imple(link_type tar, link_type tarLeft) {
    ...
    if (tarLeft->right) tarLeft->right->p = tar;
