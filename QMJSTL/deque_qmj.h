...
class deque_const_iterator {
 public:
  ...
  self operator+(const difference_type n) const { return self(node + n); }
  ...
  difference_type operator-(const self& x) const { return node - x.node; }
...
template <typename value_type>
class deque_iterator : public deque_const_iterator<value_type> {
 public:
  typedef std::random_access_iterator_tag iterator_category;
  typedef value_type value_type;
  ...
  void iter_swap(self& right) noexcept { std::iter_swap(*node, *(right.node)); }
...
class deque {
 public:
  ...
  deque() : map(nullptr), first(nullptr), last(nullptr), end_storage(nullptr) {}

  deque(const size_type n) : deque() { resize(n); }

  deque(const size_type n, const value_type& val) : deque() { assign(n, val); }
  ...
  deque(const self& x) : deque() { insert(end(), x.begin(), x.end()); }

  deque(self&& x)
      : map(x.map),
        first(x.first),
        last(x.last),
        end_storage(x.end_storage) noexcept {
  ...
  void resize(const size_type n) {
    ...
    if (n <= old_size) {
      ...
      for (map_type cur = new_last; cur != last; ++cur) destory_ele(cur);
      last = new_last;
    } else {
  ...
  iterator erase(iterator bg, iterator ed) {
    ...
    for (map_type cp = cur; cp != finish; ++cp) destory_ele(cp);
  ...
  void pop_back() { destory_ele(--last); }

  void pop_front() { destory_ele(first++); }

  void clear() {
    for (; first != last; ++first) destory_ele(first);
    last = first = map + (end_storage - map) / 2;
  }

  reference at(const size_type n) { return (*first[n]); }

  const_reference at(const size_type n) const { return (*first[n]); }
  ...
  template <typename Iter>
  typename enable_if<is_iterator<Iter>::value, void>::type assign(Iter bg,
                                                                  Iter ed) {
    clear();
    insert(end(), bg, ed);
  }
  ...
  iterator insert(iterator pos, const size_type n, const value_type& val) {
    ...
    return deallocate_and_update_ptr(size() + n, before_ele, after_ele, cur, n,
                                     val);
  }
  ...
  iterator emplace(iterator pos, types&&... args) {
    ...
    return deallocate_and_update_ptr(size() + 1, before_ele, after_ele, cur,
                                     ptr);
  }
 ...
 private:
  ...
  template <typename Iter>
  iterator insert_range(iterator pos, Iter bg, Iter ed,
  ...
  template <typename Iter>
  iterator insert_before(map_type cur, size_type before_ele, size_type n,
                         Iter bg) {
  ...
  template <typename Iter>
  iterator insert_after(map_type cur, size_type before_ele, size_type after_ele,
                        size_type n, Iter bg) {
  ...
  iterator insert_before(map_type cur, map_type ptr, iterator pos) {
    my_memcpy(first - 1, first, cur - first);
    *cur = ptr;
    return (pos);
  }

  iterator insert_after(map_type cur, map_type ptr, iterator pos) {
  ...
  template <typename Iter>
  iterator deallocate_and_update_ptr(size_type new_size, size_type before_ele,
  ...
  iterator deallocate_and_update_ptr(size_type new_size, size_type bofore_ele,
                                     map_type cur, size_type after_ele,
                                     map_type ptr) {
    size_type len = new_size << 1;
    map_type new_map = map_allocate(len);
    map_type new_first = new_map + (new_size >> 1);
 ...
 private:
  map_type copy_n(map_type cur, size_type n, const value_type& val) {
    for (; n != 0; --n, ++cur) *cur = create_ele(val);
    return cur;
  }

  map_type copy_n_default(map_type cur,
                          size_type n) {  // use default contructor,for resize
    for (; n != 0; --n, ++cur) {
  ...
  map_type copy_range(map_type cur, size_type n, InputIterator bg) {
    for (; n != 0; --n, ++cur, ++bg) *cur = create_ele(*bg);
    return cur;
  }
  ...
  map_type map_allocate(const size_type n) { return (map_alloc::allocate(n)); }

  void free_oldMap() { map_alloc::deallocate(map, end_storage - map); }

  map_type my_memmove(map_type dst, map_type src, const size_type n) {
    std::memmove(dst, src, sizeof(pointer) * n);
    return dst;
  }

  map_type my_memcpy(map_type dst, map_type src, const size_type n) {
...
inline void swap(_QMJ deque<value_type, alloc, NoType>& left,
                 _QMJ deque<value_type, alloc, NoType>& right) noexcept {
  left.swap(right);
}

template <typename value_type, typename alloc, typename NoType>
inline bool operator==(const _QMJ deque<value_type, alloc, NoType>& left,
...
class deque<value_type, Alloc,
            typename _QMJ
                enable_if<sizeof(value_type) < sizeof(value_type*) ||
                              (_QMJ is_pod<value_type>::value &&
                               sizeof(value_type) <= 10 * sizeof(value_type*)),
                          void>::type> {
 public:
  typedef value_type value_type;
  typedef value_type* pointer;
  ...
  void push_front(const value_type& val) { insert_front(val); }

  void push_front(value_type&& val) {
    emplace_front(std::forward<value_type>(val));
  }

  void push_back(const value_type& val) { insert_back(val); }
 ...
 private:
  ...
  template <typename... types>
  iterator insert_before(map_type cur, iterator pos, types&&... args) {
    my_memcpy(first - 1, first, cur - first);
  ...
  template <typename... types>
  iterator insert_after(map_type cur, iterator pos, types&&... args) {
    my_memove(cur + 1, cur, last - cur);
    alloc::construct(cur, std::forward<types>(args)...);
    return (pos);
  }
  ...
  pointer get_node(iterator pos) { return (first + (pos - begin())); }

  void clear_for_assign() { first = last = map; }

 private:
  map_type map_allocate(const size_type n) { return alloc::allocate(n); }

  void free_oldMap() { alloc::deallocate(map, end_storage - map); }
