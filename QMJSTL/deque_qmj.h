...
public:
  ...
  reference operator[](difference_type off) const { return (**(this->node + off)); }
  ...
  self operator+(const difference_type n) const { return self(this->node + n); }
  ...
  difference_type operator-(const self &x) const { return this->node - x.node; }
...
namespace qmj
{
template <typename value_type_, typename Alloc>
class deque<value_type_, Alloc,
            typename _QMJ
                enable_if<sizeof(value_type_) < sizeof(value_type_ *) ||
                              (_QMJ is_pod<value_type_>::value &&
                               sizeof(value_type_) <= 10 * sizeof(value_type_ *)),
                          void>::type>
{
public:
  typedef value_type_ value_type;
  ...
  iterator emplace(iterator pos, types &&... args)
  {
    ...
    {
      if (after_storage != last)
        return insert_after(cur, pos, std::forward<types>(args)...);
      else if (before_storage >= this->n)
