...
namespace qmj
{
	...
	public:
		...
		{
			return const_iterator(nil->right);
		}
		...
		size_type erase(const key_type&k)
		{
			...
			size_type count = 0;
			for (iterator first = range.first, last = range.second;
				first != last;++count)
				first=erase(first);
			return (count);
		}
		...
		{
			return insert_unique_imple
			(get_root(), std::forward<value_type>(value));
		}
		...
		{
			return insert_equal_imple
			(get_root(), std::forward<value_type>(val));
		}
		...
		{
			return insert_equal_imple
			(get_root(), std::forward<types>(args)...);
		}
		...
		template<bool multi = is_multi, typename...types>
		enable_if_t<!multi, iterator>
			emplace_hint(const_iterator pos, types&&...args)
		{
			...
			{
				if (cur == nil->right ||
					comp(get_key(*--pos), get_key(value)))
				{
					if (cur->left == nil)
						tar->p = cur;
					else
		...
		enable_if_t<!multi, iterator>
			emplace_hint(const_iterator pos, value_type&&value)
		{
			link_type cur = pos.get_node();
			if (cur == nil)
			{
				if (comp(get_key(back()), get_key(value)))
				{
					link_type tar = create_insert_node(nil->left, std::forward<value_type>(value));
					return (insert_imple(tar->p, tar));
				}
			}
			else if (comp(get_key(value), get_key(*pos)))
		...
		enable_if_t<multi, iterator>
			emplace_hint(const_iterator pos, types&&...args)
		{//using args to construct value_type try to insert before pos
			link_type tar = create_insert_node(nil, std::forward<types>(args)...);
			const value_type&value = tar->value;
			link_type cur = pos.get_node();
			if (!empty())
			{//tree is not empty
				...
				{//insert after max ele
					if (!comp(get_key(value), get_key(back())))
				...
				{//pos may be is a good pos to insert straightly before
					if (cur == nil->right)
					{
						tar->p = cur;
						cur->left = tar;
	...
	protected:
		...
		{
			alloc_type::deallocate((base_link_type)nil);
		}
		...
		template<typename value_type>
		iterator insert_euqal_imple
		(link_type cur, value_type&&value)
		{
			link_type par = nil;
			while (cur != nil)
			{
		...
		template<typename...types>
		iterator insert_equal_imple
		(link_type cur, types&&...args)
		{
			link_type par = nil;
		...
		{
			return (nil->right->value);
		}

		reference back()const
		{
			return (nil->left->value);
		}
	...
	template<typename traits>inline
		bool operator==(const rb_tree<traits>&left,
