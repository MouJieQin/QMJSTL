...
namespace qmj
{
	...
	public:
		...
		binary_heap(Iter first, Iter last) :
			heap_v(first, last)
		{
			build_heap();
		}
		...
		template<typename Iter>
		binary_heap
		(Iter first, Iter last, const Compare& cmp) :
		...
		template<typename Iter>
		binary_heap
		(Iter first, Iter last, const Container&cont, const Compare&comp) :
		...
		binary_heap
		(const Container&cont, const Compare&comp = Compare()) :
			heap_v(cont), comp(comp)
		{
			build_heap();
		}

		explicit binary_heap(const Compare& cmp) :
			heap_v(), comp(cmp)
		{
		}
		...
		void change_key
		(const size_t index, const value_type& val)
	...
	protected:
		...
		void change_key_imple(size_t index, const value_type&val)
		{
			for (size_t par_index = parent(index);
				index&&comp(heap_v[par_index], val);
				par_index = parent(index))
			{
	...
	template<typename value_type, typename Container,
		...
		void swap(binary_heap<value_type, Container, Compare>&left,
...
namespace qmj
{
	...
	public:
		...
		fib_heap() :
			n(0), header(nullptr), comp()
		{
		}

		fib_heap(const Compare&comp) :
			n(0), header(nullptr), comp(comp)
		{
		}
...
private:
	...
	template<typename value_type,
		...
		void swap(fib_heap<value_type, Compare, Alloc>&left,
