...
namespace qmj
{
	...
	class vector_iterator :
		public vector_const_iterator<value_type>
	{//iterator of vector
	public:
		typedef std::random_access_iterator_tag iterator_category;
		typedef value_type value_type;
	...
	public:
		...
		{//erase and destroy ele of [bg,ed) 
		...
		{//assign ele in [bg,ed)
			clear();
			assign_imple(bg, ed, _QMJ iterator_traits<Iter>::iterator_category());
		}
		...
		{
			return insert_imple(pos, bg, ed, _QMJ iterator_traits<Iter>::iterator_category());
		}
	...
	private:
		...
		{//insert [bg,ed) before pos if bg is forward iterator
			const size_type off = pos - begin();//offset
			const size_type count = _QMJ distance(bg, ed);//for calculate new space if need
		...
		{//destroy old data and dallocate old space ,update pointer for new space
			alloc::destroy(first, last);
			alloc::deallocate(first);
			first = new_first;
			last = new_last;
	...
	template<typename value_type,
		...
		vector<value_type, alloc>::insert(iterator pos,
			const size_t n, const value_type&val)
	...
	template<typename value_type, typename alloc>inline
		void swap(_QMJ vector<value_type>&left,
	...
	template<typename value_type, typename alloc>inline
		bool operator==(const _QMJ vector<value_type, alloc>&left,
