...
namespace qmj
{
		...
		struct uset_traits
	{
		...
		static const key_type& ExtractKey(const value_type&val)
		{
			return (val);
		}
	...
	template<typename key_type,
		...
		class  unordered_set :
		public hashtable<uset_traits<key_type, HashFunction,
		EqualKey,Alloc, false>>
	{
	public:
		typedef hashtable <uset_traits<key_type, HashFunction,
			EqualKey,Alloc, false>> base_type;
		typedef unordered_set<key_type, HashFunction, EqualKey> self;
	...
	public:
		...
		unordered_set(const size_type n,
			const hasher& hf,
			const key_equal& eql) :
			base_type(n, hf, eql)
		{
		...
		unordered_set(IIter first, IIter last) :
			base_type()
		{
	...
	template<typename key_type,
		...
		typename EqualKey,
		typename Alloc>
		void swap(unordered_set<key_type, HashFunction, EqualKey, Alloc>&left,
	...
	public:
		typedef hashtable <uset_traits<key_type, HashFunction,
			EqualKey,Alloc, true>> base_type;

		typedef unordered_multiset<key_type, HashFunction, EqualKey> self;
	...
	public:
		...
		unordered_multiset(const size_type n,
			const hasher& hf,
			const key_equal& eql) :
			base_type(n, hf, eql)
		{
		...
		template<typename IIter>
		unordered_multiset(IIter first, IIter last) :
			base_type()
		{
			base_type::insert(first, last);
		}
		...
		unordered_multiset(const self&x) :
			base_type(x)
		{
		}
	...
	template<typename key_type,
		...
		void swap(unordered_multiset<key_type, HashFunction, EqualKey, Alloc>&left,
			unordered_multiset<key_type, HashFunction, EqualKey, Alloc>&right)noexcept
	{
		left.swap(right);
	}
