...
struct splay_tree_base_node {
  ...
  splay_tree_base_node(link_type left = nullptr, link_type right = nullptr)
...
template <typename value_type>
struct splay_tree_node : public splay_tree_base_node<value_type> {
  typedef splay_tree_base_node<value_type> base_type;
  typedef splay_tree_node<value_type>* link_type;
  splay_tree_node(const value_type& val, link_type left = nullptr,
                  link_type right = nullptr)
      : value(val), base_type(left, right) {}

  splay_tree_node(value_type&& val, link_type left = nullptr,
                  link_type right = nullptr)
      : value(std::move(val)), base_type(left, right) {}

  splay_tree_node(link_type left = nullptr, link_type right = nullptr)
...
class splay_tree {
 public:
  ...
  link_type insert(const value_type& val) { return (insert_unique(val)); }

  link_type find(const value_type& val) { return (find_imple(val)); }

  void erase(link_type node) { delete_node(node); }
  ...
  void inorder_walk(int) {  // no stack,Morris遍历
    link_type par = root;
    link_type left;
    while (par != nil) {
      left = par->left;
      if (left == nil) {
        ...
        link_type tmp = left;
        while (tmp->right != nil && tmp->right != par) tmp = tmp->right;
        if (tmp->right == nil) {
          tmp->right = par;
          par = left;
        } else {
 ...
 private:
  link_type minimum(link_type node) {
    for (link_type cur = node->left; cur != nil; cur = node->left) node = cur;
    return (node);
  }

  link_type maximum(link_type node) {
    for (link_type cur = node->right; cur != nil; cur = node->right) node = cur;
    return (node);
  }
  ...
  PairLB splay(const value_type& val, link_type cur) {
    ...
    for (;;) {
      if (val < cur->value) {
        if (val < cur->left->value) cur = left_rotate(cur);
        if (cur->left == nil) break;
        right_min->left = cur;
        right_min = cur;
        cur = cur->left;
      } else if (cur->value < val) {
        if (cur->right->value < val) cur = right_rotate(cur);
  ...
  void delete_node(link_type node) {
    ...
    if (root->left == nil)
      ...
      new_root = root->left;
      new_root =
          splay(node->value, new_root).first;  // new_node=maximum(root->left)
