...
inline Iter next(Iter cur, _QMJ iter_dif_t<Iter> dif) {
...
template <typename Iter>
inline Iter prev(Iter cur, _QMJ iter_dif_t<Iter> dif) {
  _QMJ advance(cur, dif);
  return (cur);
}
...
struct has_mem_iter_swap : _QMJ false_type {};

template <typename FIter>
struct has_mem_iter_swap<
    FIter, void_t<decltype(std::declval<FIter>().iter_swap(FIter()))>>
    : _QMJ true_type {};

template <typename FIter>
inline void _iter_swap_imple(FIter left, FIter right, _QMJ false_type) {
  std::iter_swap(left, right);
}

template <typename FIter>
inline void _iter_swap_imple(FIter left, FIter right, _QMJ true_type) {
  left.iter_swap(right);
}

template <typename FIter1, typename FIter2>
inline void _iter_swap_same(FIter1 left, FIter2 right, _QMJ true_type) {
  _QMJ _iter_swap_imple(left, right, has_mem_iter_swap<FIter1>());
}

template <typename FIter1, typename FIter2>
inline void _iter_swap_same(FIter1 left, FIter2 right, _QMJ false_type) {
  std::iter_swap(left, right);
}
...
inline iter_val_t<Iter> max(Iter first, Iter last, const Comp &cmp) {
...
template <typename Iter>
inline iter_val_t<Iter> max(Iter first, Iter last) {
  return (_QMJ max(first, last, std::less<>()));
}

template <typename Iter, typename Comp>
inline iter_val_t<Iter> min(Iter first, Iter last, const Comp &cmp) {
  iter_val_t<Iter> val = *first;
  for (; ++first != last;)
...
template <typename Iter>
iter_val_t<Iter> inline min(Iter first, Iter last) {
  return (_QMJ min(first, last, std::less<>()));
}

template <typename Iter1, typename Iter2, typename Pred>
inline std::pair<Iter1, Iter2> mismatch(Iter1 first1, Iter1 last1, Iter2 first2,
                                        const Pred &pred) {
...
std::pair<Iter1, Iter2> inline mismatch(Iter1 first1, Iter1 last1,
                                        Iter2 first2) {
...
inline OIter set_union(IIter1 first1, IIter1 last1, IIter2 first2, IIter2 last2,
...
inline OIter set_union(IIter1 first1, IIter1 last1, IIter2 first2, IIter2 last2,
                       OIter dest) {
  return (_QMJ set_union(first1, last1, first2, last2, dest, std::less<>()));
}

template <typename IIter1, typename IIter2, typename OIter, typename Comp>
inline OIter set_intersection(IIter1 first1, IIter1 last1, IIter2 first2,
                              IIter2 last2, OIter dest, const Comp &cmp) {
  if (first1 != last1 && first2 != last2) {
    for (;;) {
      if (cmp(*first1, *first2)) {
        if (++first1 == last1) break;
...
template <typename IIter1, typename IIter2, typename OIter>
inline OIter set_intersection(IIter1 first1, IIter1 last1, IIter2 first2,
                              IIter2 last2, OIter dest) {
  return (
      _QMJ set_intersection(first1, last1, first2, last2, dest, std::less<>()));
}

template <typename IIter1, typename IIter2, typename OIter, typename Comp>
inline OIter set_difference(IIter1 first1, IIter1 last1, IIter2 first2,
                            IIter2 last2, OIter dest, const Comp &cmp) {
  if (first1 != last1 && first2 != last2) {
    for (;;) {
      if (cmp(*first1, *first2)) {
...
inline OIter set_difference(IIter1 first1, IIter1 last1, IIter2 first2,
                            IIter2 last2, OIter dest) {
  return (
      _QMJ set_difference(first1, last1, first2, last2, dest, std::less<>()));
}

template <typename IIter1, typename IIter2, typename OIter, typename Comp>
inline OIter set_symmetric_difference(IIter1 first1, IIter1 last1,
...
inline OIter set_symmetric_difference(IIter1 first1, IIter1 last1,
                                      IIter2 first2, IIter2 last2, OIter dest) {
  return (_QMJ set_symmetric_difference(first1, last1, first2, last2, dest,
                                        std::less<>()));
}

template <typename FIter, typename Pred>
inline FIter adjacent_find(FIter first, FIter last, const Pred &pred) {
  if (first != last) {
...
template <typename FIter>
inline FIter adjacent_find(FIter first, FIter last) {
  return (_QMJ adjacent_find(first, last, std::equal_to<>()));
}

template <typename Iter, typename Pred>
inline iter_dif_t<Iter> count_if(Iter first, Iter last, const Pred &pred) {
...
iter_dif_t<Iter> inline count(Iter first, Iter last, const value_type &val) {
  iter_dif_t<Iter> counter = 0;
  for (; first != last; ++first)
    if (*first == val) ++counter;
  return (counter);
}

template <typename FIter, typename Pred>
FIter _partition_imple(FIter first, FIter last, const Pred &pred,
  ...
  for (FIter next = first; ++next != last;)
    if (pred(*next)) _QMJ iter_swap(first++, next);

  return (first);
}

template <typename BIter, typename Pred>
BIter _partition_imple(BIter first, BIter last, const Pred &pred,
...
inline FIter1 _search_imple(FIter1 first1, FIter1 last1, FIter2 first2,
                            FIter2 last2, const Pred &pred,
                            std::forward_iterator_tag,
                            std::forward_iterator_tag) {
...
inline FIter1 search(FIter1 first1, FIter1 last1, FIter2 first2, FIter2 last2,
                     const Pred &pred) {
  return (_QMJ _search_imple(first1, last1, first2, last2, pred,
                             iter_cate_t<FIter1>(), iter_cate_t<FIter2>()));
}

template <typename FIter1, typename FIter2>
inline FIter1 search(FIter1 first1, FIter1 last1, FIter2 first2, FIter2 last2) {
  return (_QMJ search(first1, last1, first2, last2, std::equal_to<>()));
}

template <typename FIter, typename Dif, typename value_type, typename Pred>
inline FIter _search_n_imple(FIter first, FIter last, Dif n,
                             const value_type &val, const Pred &pred,
                             std::forward_iterator_tag) {
  for (; first != last; ++first) {
    if (pred(*first, val)) {  // first may be result
      FIter cur = first;
      ++cur;
...
inline FIter _search_n_imple(FIter first, FIter last, Dif n,
  ...
  for (Dif mark = n - 1; mark < count; mark += n) {
    if (pred(first[mark], val)) {  //[ret,first+mark] may be satisifyed
      for (Dif cur = 0;; ++cur)
        if (cur == n)
          return (result);
        else if (!pred(result[cur], val)) {  // adjust mark
          mark -= (n - cur - 1);
          break;
        }
    }
...
inline FIter search_n(FIter first, FIter last, Dif n, const value_type &val,
                      const Pred &pred) {
  return (
      _QMJ _search_n_imple(first, last, n, val, pred, iter_cate_t<FIter>()));
}

template <typename FIter, typename Dif, typename value_type>
inline FIter search_n(FIter first, FIter last, Dif n, const value_type &val) {
  return (_QMJ search_n(first, last, n, val, std::equal_to<>()));
}

template <typename Iter, typename value_type>
inline Iter find(Iter first, Iter last, const value_type &val) {
  for (; first != last && *first != val;) ++first;
  return (first);
}

template <typename Iter, typename Pred>
inline Iter find_if(Iter first, Iter last, const Pred &pred) {
  for (; first != last && (!pred(*first));) ++first;
  return (first);
}

template <typename FIter1, typename FIter2, typename Pred>
inline FIter1 _find_end_imple(FIter1 first1, FIter1 last1, FIter2 first2,
                              FIter2 last2, const Pred &pred,
                              std::forward_iterator_tag,
                              std::forward_iterator_tag) {
  FIter1 result = last1;
  FIter1 new_result;
  while (true) {
    new_result = _QMJ search(first1, last1, first2, last2, pred);
    if (new_result == last1)
      return (result);
    else {
      result = new_result;
...
inline BIter1 _find_end_imple(BIter1 first1, BIter1 last1, BIter2 first2,
                              BIter2 last2, const Pred &pred,
                              std::bidirectional_iterator_tag,
                              std::bidirectional_iterator_tag) {
  ...
  RBIter1 rresult =
      _QMJ search(RBIter1(last1), rlast1, RBIter2(last2), rlast2, pred);
  if (rresult == rlast1)
    return (last1);
  else {
...
inline Iter1 find_end(Iter1 first1, Iter1 last1, Iter2 first2, Iter2 last2,
                      const Pred &pred) {
  return (_QMJ _find_end_imple(first1, last1, first2, last2, pred,
                               iter_cate_t<Iter1>(), iter_cate_t<Iter2>()));
}

template <typename Iter1, typename Iter2>
inline Iter1 find_end(Iter1 first1, Iter1 last1, Iter2 first2, Iter2 last2) {
  return (_QMJ find_end(first1, last1, first2, last2, std::equal_to<>()));
}

template <typename FIter1, typename FIter2, typename Pred>
inline FIter1 find_first_of(FIter1 first1, FIter1 last1, FIter2 first2,
                            FIter2 last2, const Pred &pred) {
  for (; first1 != last1; ++first1)
...
inline FIter1 find_first_of(FIter1 first1, FIter1 last1, FIter2 first2,
                            FIter2 last2) {
  return (_QMJ find_first_of(first1, last1, first2, last2, std::equal_to<>()));
}

template <typename Iter, typename Fn1>
inline Fn1 for_earch(Iter first, Iter last, const Fn1 &fn1) {
  for (; first != last; ++first) fn1(*first);
  return (fn1);
}

template <typename FIter, typename Fn0>
inline void generate(FIter first, FIter last, const Fn0 &fn0) {
  for (; first != last; ++first) *first = fn0();
}

template <typename OIter, typename Dif, typename Fn0>
...
inline bool includes(IIter1 first1, IIter1 last1, IIter2 first2, IIter2 last2) {
  return (_QMJ includes(first1, last1, first2, last2, std::less<>()));
}
...
template <typename FIter, typename Comp>
inline FIter min_element(FIter first, FIter last, const Comp &cmp) {
  FIter result = first;
  if (first != last) {
    for (iter_val_t<FIter> val = *first; ++first != last;)
      if (cmp(*first, val)) {
...
template <typename IIter1, typename IIter2, typename OIter, typename Comp>
inline OIter merge(IIter1 first1, IIter1 last1, IIter2 first2, IIter2 last2,
                   OIter dest, const Comp &cmp) {
  if (first1 != last1 && first2 != last2) {
    for (;;) {
      if (cmp(*first2, *first1)) {
...
inline Iter remove(Iter first, Iter last, const value_type &val) {
  ...
  if (first != last) {
    for (Iter next = first; ++next != last;)
      if (*next != val) *first++ = std::move(*next);
  }
  return (first);
}

template <typename Iter, typename OIter, typename value_type>
inline OIter remove_copy(Iter first, Iter last, OIter dest,
...
inline OIter remove_copy_if(IIter first, IIter last, OIter dest,
                            const Pred &pred) {
  for (; first != last; ++first)
...
inline void replace(FIter first, FIter last, const value_type &old_val,
...
inline OIter replace_copy(FIter first, FIter last, OIter dest,
...
inline void replace_if(FIter first, FIter last, const Pred &pred,
                       const value_type &val) {
  for (; first != last; ++first)
    if (pred(*first)) *first = val;
}

template <typename IIter, typename OIter, typename Pred, typename value_type>
inline OIter replace_copy_if(IIter first, IIter last, OIter dest,
                             const Pred &pred, const value_type &val) {
  for (; first != last; ++first, ++dest) *dest = pred(*first) ? val : *first;
  return (dest);
}

template <typename BIter>
inline void reverse(BIter first, BIter last) {
  for (; first != last && first != --last; ++first) _QMJ iter_swap(first, last);
}
...
inline void rotate(Iter first, Iter middle, Iter last) {
  _QMJ _rotate_imple(first, middle, last, iter_cate_t<Iter>());
}
...
inline FIter2 swap_ranges(FIter1 first1, FIter1 last1, FIter2 first2) {
  for (; first1 != last1; ++first1, ++first2) _QMJ iter_swap(first1, first2);
  return (first2);
}

template <typename IIter, typename OIter, typename Fn1>
inline OIter transform(IIter first, IIter last, OIter dest, const Fn1 &fn1) {
  for (; first != last; ++first, ++dest) *dest = fn1(*first);
  return (dest);
}

template <typename IIter1, typename IIter2, typename OIter, typename Fn2>
inline OIter transform(IIter1 first1, IIter1 last1, IIter2 first2, OIter dest,
                       const Fn2 &fn2) {
...
template <typename FIter, typename OIter>
inline OIter unique_copy(FIter first, FIter last, OIter dest) {
  return (_QMJ unique_copy(first, last, dest, std::equal_to<>()));
}
...
template <typename Iter, typename value_type>
inline Iter lower_bound(Iter first, Iter last, const value_type &val) {
  return (_QMJ lower_bound(first, last, val, std::less<>()));
}
...
inline bool binary_search(FIter first, FIter last, const value_type &val,
                          const Comp &cmp) {
  FIter result =
      lower_bound(first, last, val, cmp);  //*result>=val if result!=last
...
inline bool next_permutation(BIter first, BIter last, const Comp &cmp) {
  BIter before = last;
  if (first == last || --before == first) return (false);
...
inline bool next_permutation(BIter first, BIter last) {
  return (_QMJ next_permutation(first, last, std::less<>()));
}

template <typename BIter, typename Comp>
inline bool prev_permutation(BIter first, BIter last, const Comp &cmp) {
  BIter before = last;
  if (before == first || --before == first) return (false);
...
inline bool prev_permutation(BIter first, BIter last) {
  return (_QMJ prev_permutation(first, last, std::less<>()));
}
...
template <typename BIter1, typename BIter2, typename Pred>
inline void _match_suffixes(BIter1 &last1, BIter2 &last2, const Pred &pred,
                            std::bidirectional_iterator_tag,
                            std::bidirectional_iterator_tag) {
...
inline Iter _find_fn2(Iter first, Iter last, const value_type &val,
                      const Pred &pred) {
  for (; first != last; ++first)
...
template <typename Iter, typename value_type, typename Pred>
inline iter_dif_t<Iter> _count_fn2(Iter first, Iter last, const value_type &val,
                                   const Pred &pred) {
  iter_dif_t<Iter> counter = 0;
  for (; first != last; ++first)
...
template <typename Iter1, typename Iter2, typename Pred>
inline bool _match_count(Iter1 first1, Iter1 last1, Iter2 first2, Iter2 last2,
                         const Pred &pred) {
  _QMJ _match_suffixes(last1, last2, pred, iter_cate_t<Iter1>(),
                       iter_cate_t<Iter2>());
  Iter1 skip;
...
template <typename FIter1, typename FIter2, typename Pred>
inline bool _is_permutation_imple(FIter1 first1, FIter1 last1, FIter2 first2,
                                  const Pred &pred) {
...
inline bool _is_permutation_imple(RIter1 first1, RIter1 last1, RIter2 first2,
                                  RIter2 last2, const Pred &pred,
                                  std::random_access_iterator_tag,
                                  std::random_access_iterator_tag) {
...
inline bool is_permutation(Iter1 first1, Iter1 last1, Iter2 first2, Iter2 last2,
                           const Pred &pred) {
  return (_QMJ _is_permutation_imple(first1, last1, first2, last2, pred,
                                     iter_cate_t<Iter1>(),
                                     iter_cate_t<Iter2>()));
}
...
inline bool is_permutation(Iter1 first1, Iter1 last1, Iter2 first2,
                           Iter2 last2) {
  return (_QMJ is_permutation(first1, last1, first2, last2, std::equal_to<>()));
}
...
template <typename Iter1, typename Iter2>
inline bool is_permutation(Iter1 first1, Iter1 last1, Iter2 first2) {
  return (_QMJ is_permutation(first1, last1, first2, std::equal_to<>()));
}

template <typename RIter, typename Dif, typename value_type, typename Comp>
inline void _push_heap_imple(
    RIter first, Dif index, value_type &&val,
    const Comp &cmp) {  // percolate index to where val belong,using cmp
  for (Dif par_index = (size_t)(index - 1) >> 1;
       index && cmp(*(first + par_index), val);
       par_index = (size_t)(index - 1) >> 1) {  // move val up to parent
...
inline void push_heap(
    RIter first,
    RIter last) {  // push *(last-1) int heap [first,last-1),using <
  _QMJ push_heap(first, last, std::less<>());
}

template <typename RIter, typename Comp>
inline void push_heap(
    RIter first, RIter last,
    const Comp &cmp) {  // push *(last-1) int heap [first,last-1),using cmp
  _QMJ _push_heap_imple(first, iter_dif_t<RIter>(last - first - 1),
                        std::move(*(last - 1)), cmp);
}

template <typename RIter, typename Dif, typename value_type, typename Comp>
inline void _heapify(RIter first, Dif index, const Dif len, value_type &&val,
                     const Comp &cmp) {  // percolate down,put val in a good
                                         // node in heap of
                                         // [first,first+len),using cmp
  ...
  if (right_index == len &&
      cmp(val, *(first +
                 (--right_index)))) {  // index have a left node and satisfy cmp
...
inline void _pop_heap_imple(RIter first, RIter last, const Comp &cmp) {
  ...
  *last = std::move(*first);
  _QMJ _heapify(first, iter_dif_t<RIter>(0), iter_dif_t<RIter>(last - first),
                std::move(val), cmp);
}
...
inline void pop_heap(
    RIter first, RIter last,
    const Comp &cmp) {  // pop *first to *(last - 1) and reheap, using cmp
  iter_dif_t<RIter> dif = last - first;
  if (dif >= 2) _QMJ _pop_heap_imple(first, last, cmp);
}

template <typename RIter>
inline void pop_heap(
    RIter first, RIter last) {  // pop *first to *(last - 1) and reheap, using <
  _QMJ pop_heap(first, last, std::less<>());
}

template <typename RIter>
inline void sort_heap(RIter first,
                      RIter last) {  // sort the heap of [first,last) ,using <
...
inline void sort_heap(
    RIter first, RIter last,
    const Comp &cmp) {  // sort the heap of [first,last) ,using cmp
  for (; last - first >= 2; --last) _QMJ _pop_heap_imple(first, last, cmp);
}
...
template <typename RIter, typename Comp>
inline void make_heap(RIter first, RIter last,
                      const Comp &cmp) {  // make heap in [first,last),using cmp
  const iter_dif_t<RIter> len = last - first;
  if (len >= 2) _QMJ _make_heap_imple(first, len, cmp);
}

template <typename RIter>
void make_heap(RIter first, RIter last) {  // make heap in [firs,last),using <
  _QMJ make_heap(first, last, std::less<>());
}

template <typename RIter, typename Comp>
inline void partial_sort(RIter first, RIter middle, RIter last,
                         const Comp &cmp) {
...
inline RIter partial_sort_copy(IIter first, IIter last, RIter dest_first,
                               RIter dest_last, const Comp &cmp) {
...
inline RIter partial_sort_copy(IIter first, IIter last, RIter dest_first,
                               RIter dest_last) {
  return (_QMJ partial_sort_copy(first, last, dest_first, dest_last,
                                 std::less<>()));
}

template <typename BIter, typename value_type, typename Comp>
inline void _unguarded_linear_insert(BIter last, value_type val,
                                     const Comp &cmp) {
...
template <typename BIter, typename Comp>
inline void _unguarded_insert_sort(BIter first, BIter last, const Comp &cmp) {
  for (BIter i = first; i != last; ++i)
    _unguarded_linear_insert(i, std::move(*i), cmp);
}

template <typename BIter, typename Comp>
inline void _linear_insert(BIter first, BIter last, const Comp &cmp) {
...
inline void _insert_sort_imple(BIter first, BIter last, const Comp &cmp) {
  for (BIter cur = first; ++cur != last;) _linear_insert(first, cur, cmp);
}
...
template <typename RIter, typename Comp>
inline void _introsort_imple(RIter first, RIter last, size_t depth_limit,
                             const Comp &cmp) {
...
inline void sort(RIter first, RIter last, const Comp &cmp) {
  if (last - first > sort_threshold)
    _introsort_imple(first, last, size_t(_lg2((size_t)(last - first)) * 1.5),
                     cmp);
...
inline std::pair<RIter, RIter> equal_range(RIter first, RIter last,
                                           const value_type &val,
                                           const Comp &cmp) {
...
inline BIter1 _rotate_adaptive(BIter1 first, BIter1 middle, BIter1 last,
...
inline void _merge_backward(BIter1 first1, BIter1 last1, BIter2 first2,
                            BIter2 last2, OIter dest, const Comp &cmp) {
  if (first1 != last1 && first2 != last2) {
    for (--last1, --last2;;) {
      if (cmp(*last2, *last1)) {
        *--dest = *last1;
...
inline void _merge_adaptive(BIter first, BIter middle, BIter last, Dif len1,
                            Dif len2, pointer buf, Dif buf_size,
                            const Comp &cmp) {
  if (len1 <= len2 && len1 <= buf_size) {
    ...
    BIter new_middle = _QMJ _rotate_adaptive(
        first_cut, middle, second_cut, len1 - len11, len22, buf, buf_size);
    _QMJ _merge_adaptive(first, first_cut, new_middle, len11, len22, buf,
                         buf_size, cmp);
    _QMJ _merge_adaptive(new_middle, second_cut, last, len1 - len11,
                         len2 - len22, buf, buf_size, cmp);
  }
}

template <typename BIter, typename Comp>
inline void _inplace_merge_imple(BIter first, BIter middle, BIter last,
                                 const Comp &cmp) {
  ...
  _QMJ _merge_adaptive(first, middle, last, len1, len2, buf.begin(),
...
template <typename BIter, typename Dif, typename pointer, typename Comp>
inline void _stable_sort_imple(BIter first, BIter last, pointer buf,
                               Dif buf_size, const Comp &cmp) {
  ...
  if (len > sort_threshold) {
    ...
    _QMJ _merge_adaptive(first, middle, last, len1, len2, buf, buf_size, cmp);
  } else
    _insert_sort(first, last, cmp);
}
