...
public:
  ...
  { // redistribution size,use default of value_type
    // supplement
  ...
  iterator insert_after(const_iterator pos, size_type n,
                        const value_type &val)
  ...
  insert_after(const_iterator pos, Iter first,
               Iter last)
  ...
  void unique()
  { // default remove a element if adjance two element is
  ...
  { // remove a element if adjance two element is
    // satisfied pred
    link_type before = b_head->next;
    if (before)
    {
