...
namespace qmj {
template <int inst>
class malloc_alloc_template {  //һ����������ֱ��ʹ��malloc/free������ͷ��ڴ�
 public:  //��ģ���׼���ڴ治�㴦����
...
class default_alloc_template {  //�����������������ڴ�ص�ʵ��
 private:
...
typename default_alloc_template<threads, inst>::
    obj* volatile default_alloc_template<threads, inst>::free_list[nfreelists] =
...
template <typename value_type_, typename allocator_type>
class allocator_base {  //���������࣬����allocator_type ����������һ�η�����
 public:
  ...
  inline static pointer allocate(const size_type n) {
    return (n ? static_cast<pointer>(alloc::allocate(sizeof(value_type) * n))
              : nullptr);
  }

  template <typename... types>
  inline static void construct(pointer ptr, types&&... args) {
    new (ptr) value_type(std::forward<types>(args)...);
  }

  inline static void construct(pointer ptr) { new (ptr) value_type(); }
  ...
  inline static void copy_construct(type* ptr, const type& val) {
    copy_construct_imple(
        ptr, val,
        typename qmj::type_traits<type>::has_trivial_copy_constructor());
  }

  inline static void copy_construct(pointer ptr, const value_type& val) {
    copy_construct_imple(
        ptr, val,
        typename qmj::type_traits<value_type>::has_trivial_copy_constructor());
  }
  ...
  inline static pointer copy_construct(pointer first, pointer last,
  ...
  template <typename Iter>
  inline static pointer copy_construct(Iter first, const size_type distance,
                                       pointer dest) {
    return copy_construct_imple(
        first, distance, dest,
        typename qmj::type_traits<value_type>::has_trivial_copy_constructor());
  ...
  inline static pointer copy_construct(pointer first, size_type n,
                                       const value_type& val) {
    for (; n != 0; --n, ++first) copy_construct(first, val);
    return (first);
  }
  ...
  inline static void destroy(type* ptr) {
    destroy_imple(ptr,
                  typename qmj::type_traits<type>::has_trivial_destructor());
  }
 ...
 private:
  inline static void copy_construct_imple(pointer ptr, const value_type& val,
                                          true_type) {
    *ptr = val;
  }
  ...
  template <typename type>
  inline static void copy_construct_imple(type* ptr, const type& val,
                                          true_type) {
    *ptr = val;
  }
  ...
  template <typename Iter>
  inline static pointer copy_construct_imple(Iter first, Iter last,
                                             pointer dest, false_type) {
    return copy_construct_imple_td(
        first, last, dest,
        typename qmj::iterator_traits<Iter>::iterator_category());
  }

  template <typename Iter>
  inline static pointer copy_construct_imple_td(Iter first, Iter last,
                                                pointer dest,
  ...
  template <typename Iter>
  inline static pointer copy_construct_imple_td(
      Iter first, Iter last, pointer dest, std::random_access_iterator_tag) {
    for (size_t distance = last - first; distance != 0;
         --distance, ++first, ++dest)
  ...
  template <typename Iter>
  inline static pointer copy_construct_imple_nt_d(Iter first, Iter last,
                                                  pointer dest,
                                                  std::input_iterator_tag) {
  ...
  template <typename Iter>
  inline static pointer copy_construct_imple_nt_d(
      Iter first, Iter last, pointer dest, std::random_access_iterator_tag) {
    return copy_construct_imple_nt_d_m(first, last, dest,
                                       typename qmj::is_mem_copy<Iter>());
  }

  template <typename Iter>
  inline static pointer copy_construct_imple_nt_d_m(Iter first, Iter last,
                                                    pointer dest, false_type) {
    for (size_t distance = last - first; distance != 0;
         --distance, ++first, ++dest)
      *dest = *first;
  ...
  inline static pointer copy_construct_imple_nt_d_m(Iter first, Iter last,
    ...
    memcpy(dest, &*first, sizeof(value_type) * distance);
    return (dest + distance);
  }

  template <typename Iter>
  inline static pointer copy_construct_imple(Iter first, size_type distance,
                                             pointer dest, false_type) {
  ...
  template <typename Iter>
  inline static pointer copy_construct_imple(Iter first,
                                             const size_type distance,
                                             pointer dest, true_type) {
    return copy_construct_imple_memory(dest, first, distance,
                                       qmj::is_mem_copy<Iter>());
  }

  template <typename Iter>
  inline static pointer copy_construct_imple_memory(pointer dest, Iter first,
                                                    size_type distance,
                                                    true_type) {
    memcpy(dest, (&(*first)), sizeof(value_type) * distance);
    return (dest + distance);
  ...
  static void destroy_imple(pointer ptr, false_type) { ptr->~value_type(); }

  template <typename type>
  inline static void destroy_imple(type* ptr, true_type) {}

  template <typename type>
  inline static void destroy_imple(type* ptr, false_type) {
    ptr->~type();
  }
...
class simple_allocator
    : public allocator_base<value_type_, malloc_alloc> {  //ʹ��һ��������
 public:
  typedef allocator_base<value_type_, malloc_alloc> base_type;
  typedef typename base_type::value_type value_type;
  typedef typename base_type::pointer pointer;
  typedef typename base_type::const_pointer const_pointer;
  typedef typename base_type::reference reference;
